...
func set_iso_metrics(origin: Vector2, current_tile_width: float, current_tile_height: float, current_row_step: float, size: Vector2) -> void:
	map_origin = origin
...
func _tile_center(x: int, y: int) -> Vector2:
	return map_origin + Vector2(float(x - y) * tile_width * 0.5, float(x + y) * row_step)


func _tile_rect(x: int, y: int) -> Rect2:
	var center := _tile_center(x, y)
	return Rect2(center - Vector2(tile_width * 0.5, tile_height * 0.34), Vector2(tile_width, tile_height))
