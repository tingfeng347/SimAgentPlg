...
基础运行控制协议、Provider-neutral Text / Thinking / Tool Progress 流，以及模型 Usage
聚合与单次 Run Token Budget。
...
下一步建议实现 Context Pressure Estimate 与 Compaction Preparation，但仍先不自动生成
Summary。当前 `RunUsage.total_tokens` 是多次 Provider 请求的累计消耗，会重复计算历史
上下文，不能直接当作 context window 占用。

推荐下一轮建立：

1. 从最近一次 assistant Usage 读取真实输入/输出上下文基线。
2. 对该响应之后新增的 Tool/User/System 消息做保守 token 估算。
3. 生成 `ContextUsageEstimate(reported_tokens, trailing_tokens, total_tokens)`。
4. 定义独立的 `ContextBudget` / `CompactionPolicy`，不复用 `max_run_tokens`。
5. 只完成 should-compact 判断和 cut-point preparation；Summary 生成、Session 结构变更与
   自动触发留到下一小步。
