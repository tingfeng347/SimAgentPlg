...
SimAgentPlg 已完成通用 Agent Core 的执行内核、只读事件协议、线性内存 Session 和
基础运行控制协议。
...
BaseAgent.run(task)
  ...
  → ModelAdapter.complete(context, cancellation)
  → AssistantMessage
...
下一步应在现有取消协议上实现 Provider Streaming。流式模型、未来 Tool Progress 和
ExecutionEnv 都复用同一个 per-run CancellationToken，不再建立新的停止逻辑。

推荐第一轮改动范围：

```text
src/simagentplg/providers/base.py
src/simagentplg/providers/openai.py
...
1. Provider-neutral Stream Adapter 不暴露 OpenAI SDK 类型。
2. 文本增量和最终 `AssistantMessage` 使用同一条事件序列。
3. 中途 `abort()` 不提交未完成的普通 `MessageCompleted`，但仍提交确定性终态。
4. 非流式 Model Adapter 仍可通过兼容路径工作。
5. 第一轮只实现 Text Delta；Thinking Delta 与 Tool Progress 分开演进。
