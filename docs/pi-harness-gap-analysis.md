...
聚合与单次 Run Token Budget，以及上下文压力评估和非变异压缩准备。
...
- Summary、自动 Compaction 与 Overflow Recovery
...
本阶段刻意不生成 Summary、不停止 Run、不替换历史，也不自动恢复 Provider overflow。
事件仍是只读观察协议；行为型 Compactor 留到下一阶段。
...
完整 User Turn 边界准备切分，暂不引入 mid-turn summary。更重要的是，它只发布只读
`ContextPressureEvaluated`，不会由 `AgentOrchestrator` 直接生成摘要或改写线性 Session。
这使未来 WebAgent、MCP Agent 和 CodeAgent 可以共享压力机制，但使用不同的 Compactor。
...
下一步建议实现 `SummaryEntry + Compactor Protocol`，把当前只读 Preparation 变成可插拔
行为，但先提供显式/manual compaction，不立即启用自动阈值触发。

推荐下一轮建立：

1. 定义 Provider-neutral `SummaryEntry`，保存摘要正文、覆盖范围、压缩前 Token 和来源。
2. 定义可取消的 `Compactor` 协议，让派生 Agent 决定摘要模型、Prompt 和结构。
3. 增加 Compaction Started / Completed / Failed 事件，并保持终态事件可靠性。
4. 原子地生成“受保护上下文 + Summary + 最近历史”，摘要失败时不修改 Agent State。
5. 明确 SessionRecorder 如何记录 Summary Entry 和恢复投影。
6. 先实现显式 `compact()`；其语义稳定后再把 `ContextPressureEvaluated` 接到自动触发和
   overflow retry。
