...
基础运行控制协议，以及 Provider-neutral Text / Thinking / Tool Progress 流。
...
下一步建议进入 Context Budget 基础阶段，先完成 Provider-neutral Usage 数据和预算观察，
再实现 Compaction。Tool Progress 已补齐模型与工具两侧的实时观察通道；如果没有可靠的
token/usage 事实，直接做摘要压缩会把触发阈值和 Provider 计量逻辑重新塞回
Orchestrator。

推荐第一轮只建立：

1. Provider 终态响应携带可选的标准化 Usage。
2. 每轮和每次 Run 的 Usage 事件与聚合结果。
3. `RuntimePolicy` 中独立于 `max_steps` 的可选 Token Budget。
4. complete-only Adapter 和不返回 Usage 的兼容语义。
5. 预算观察与硬停止的边界测试；暂不同时实现自动 Summary/Compaction。
