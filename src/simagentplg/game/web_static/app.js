...
function renderFactions() {
  ...
  state.factions.forEach((faction) => {
    ...
    row.innerHTML = `
      ...
      <div class="metric-line">人口 ${faction.population} · 士兵 ${faction.soldiers} · 领土 ${faction.territory_count}</div>
...
function tileDetails(tile) {
  ...
  return `
    ...
    天气：${displayWeather(tile.weather)}<br>
...
function formatDiplomacy(diplomacy) {
  return Object.entries(diplomacy)
...
function formatEvent(event) {
  ...
  match = message.match(/^God changed weather at \((\d+), (\d+)\) to (\w+)$/);
  if (match) return `上帝将（${match[1]}, ${match[2]}）的天气改为${displayWeather(match[3])}`;

  match = message.match(/^God assigned tile \((\d+), (\d+)\) from (.+) to (\w+)$/);
  if (match) {
    return `上帝将（${match[1]}, ${match[2]}）从${displayOwner(match[3])}划给${displayFaction(match[4])}`;
  }
  ...
  match = message.match(/^(\w+) captured (.+) from (\w+)$/);
  if (match) return `${displayFaction(match[1])} 从 ${displayFaction(match[3])} 手中占领 ${formatTarget(match[2])}`;
